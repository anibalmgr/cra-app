...
export default function Dashboard() {
  ...
  return (
    <div className="w-full h-full">
      <Outlet />
      <footer className="fixed bg-[white] bottom-0 right-0 left-0 md:top-0 md:right-auto w-screen md:w-20 md:h-screen border-t-2 md:border-t-0 md:border-r-2 border-x-content h-20 flex justify-center">
          ...
          {links.map((link, i) => (
            <Link key={i} to={`/${link.to}`}>
              <Icon
                name={link.icon}
                className={`flex h-7
               stroke-content hover:stroke-content-hover ${
                 isActive(`/dashboard${link.to}`)
                   ? "stroke-contrast hover:stroke-contrast-hover"
                   : ""
               }`}
              />
            </Link>
